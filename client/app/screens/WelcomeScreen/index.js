    ...
    function _renderItem({ item }) {
        return (
            <View style={styles.textWrapper}>
                ...
                <View>
                    <Text style={{ fontSize: NormalizeFont(24), color: 'white' }}>{item.title}
                        <Text style={{ color: AppStyle.third_main_color }}> {item.sub_title}</Text>
                    </Text>
                    <Text style={[
                        { fontSize: NormalizeFont(24), color: AppStyle.third_main_color },
                        activeIndex == 2 ? { display: 'flex' } : { display: 'none' }
                    ]}>{item.second_sub_title}
                        <Text style={{ color: 'white' }}> {item.second_title}</Text>
                    </Text>
                </View>
                <Text style={{ color: 'white', fontSize: NormalizeFont(14) }}>{item.text}</Text>
            </View >
        )
    }
